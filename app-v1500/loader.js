function patchApp(html){
  ...
  exact('Banco auditado · versión <b id="bankVersionLabel">2026.08.04</b>',
        'Banco auditado · 1.500 formulaciones · versión <b id="bankVersionLabel">2026.08.04.3</b>','cabecera del banco');
  exact('const VERSION="2026.08.04";','const VERSION="2026.08.04.3";','versión del banco');
  ...
  exact(countField,countField.replace('<button class="selected" type="button" data-count="20">20</button>','<button type="button" data-count="15">15</button><button class="selected" type="button" data-count="20">20</button>')+'\n      <div id="studyPoolInfo" class="pool-info" aria-live="polite"></div>','información del banco de estudio');
...
`function buildVariants(c){
 ...
 return formulations.map((question,i)=>({...c,id:\`\${c.factId}-V\${i+1}\`,variant:i+1,question}))
  ...
  exact('CONCEPTS.length!==250||BANK.length!==1000||facts.size!==250',
        'CONCEPTS.length!==250||BANK.length!==1500||facts.size!==250||ALL_TOPICS.some(t=>new Set(BANK.filter(q=>q.topic===t).map(q=>q.factId)).size<10)','auditoría de cantidad');
...
function normalizeQuestionText(text){return String(text??'').toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').replace(/[¿?¡!.,;:()"']/g,' ').replace(/\\s+/g,' ').trim()}
function groupQuestionPool(pool){const groups=new Map();pool.forEach(q=>{if(!groups.has(q.factId))groups.set(q.factId,[]);groups.get(q.factId).push(q)});return groups}
function chooseUniqueStudyQuestions(pool,count,avoidIds=new Set()){
 const groups=groupQuestionPool(pool),selected=[],usedTexts=new Set();
 for(const factId of shuffle([...groups.keys()])){if(selected.length>=count)break;const candidates=shuffle(groups.get(factId)).sort((a,b)=>(avoidIds.has(a.id)?1:0)-(avoidIds.has(b.id)?1:0));const pick=candidates.find(q=>!usedTexts.has(normalizeQuestionText(q.question)));if(pick){selected.push(pick);usedTexts.add(normalizeQuestionText(pick.question))}}
 return selected.slice(0,count)
}
function maxUniqueStudyQuestions(pool){const groups=groupQuestionPool(pool),usedTexts=new Set();let total=0;[...groups.keys()].sort().forEach(factId=>{const pick=groups.get(factId).find(q=>!usedTexts.has(normalizeQuestionText(q.question)));if(pick){total++;usedTexts.add(normalizeQuestionText(pick.question))}});return total
}
...
function customQuestions({topic='all',section='all',count=20,factIds=null}){const pool=filteredQuestionPool({topic,section,factIds});if(factIds?.length){const maxFacts=new Set(pool.map(q=>q.factId)).size;return shuffle(chooseDistinctFacts(pool,Math.min(count,maxFacts))).map(prepareQuestion)}return chooseUniqueStudyQuestions(pool,Math.min(count,maxUniqueStudyQuestions(pool)),recentStudyQuestionIds()).map(prepareQuestion)}`,
'generación única de estudio');
...
`function startStudy(){const section=$('studySection').value,topic=$('studyTopic').value,qs=customQuestions({section,topic,count:studyCount});if(qs.length!==studyCount){alert(\`Solo hay \${qs.length} preguntas independientes disponibles para esta selección.\`);return}const scope=topic!=='all'?topic:(section!=='all'?section:'Todo el temario');active=createSession('study',qs,\`Estudio · \${scope}\`,$('instantCorrection').checked);saveActive();renderExam();show('exam')}`,
...
`function studyPoolSummary(){const section=$('studySection').value,topic=$('studyTopic').value,pool=filteredQuestionPool({section,topic}),facts=new Set(pool.map(q=>q.factId)).size,available=maxUniqueStudyQuestions(pool);return {facts,available,formulations:pool.length}}
function updateStudyPoolInfo(){const {facts,available,formulations}=studyPoolSummary(),buttons=[...$('studyCount').querySelectorAll('[data-count]')];buttons.forEach(b=>b.disabled=Number(b.dataset.count)>available);if(studyCount>available){studyCount=buttons.map(b=>Number(b.dataset.count)).filter(n=>n<=available).pop()||Math.min(available,10)}buttons.forEach(b=>b.classList.toggle('selected',Number(b.dataset.count)===studyCount));$('studyPoolInfo').innerHTML=\`<strong>\${available} preguntas independientes disponibles</strong> · \${facts} conceptos auditados · \${formulations} formulaciones totales. La sesión no repite conceptos ni enunciados.\`}
